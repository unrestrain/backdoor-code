...
def train(model, target_label, train_loader, param, device='cuda:0'):
...
def detect(model, trainset, device='cuda:0'):
    param = {
        "dataset": "cifar10",
        "Epochs": 10,
        "batch_size": 64,
    ...
    for label in range(0,param["num_classes"]):
        trigger, mask = train(model, label, train_loader, param,device=device)
